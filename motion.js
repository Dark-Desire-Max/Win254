...
(() => {
  ...
  const ripple = (event) => {
    ...
    if (reduce || button.disabled) return;
  ...
  const enhance = (root = document) => {
    root.querySelectorAll('button:not([data-motion-ready])').forEach(button => {
  ...
  document.addEventListener('pointermove', event => {
    if (reduce) return;
    const card = event.target.closest('.casino-card,.promo-card');
    if (!card) return;
